...
def do_git_update():
    ...
    if success:
        mesgdcrt.SuccessMessage("CEbomb was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update CEbomb.")
        mesgdcrt.WarningMessage("Make Sure To Install 'git' ")
        mesgdcrt.GeneralMessage("Then run command:")
        print(
            "git checkout . && "
            "git pull https://github.com/Abhay110105/CEbomb.git HEAD")
    sys.exit()
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SuccessMessage("CEbomb was created by CyberEdge")
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("CEbomb will work only in Python v3")
    sys.exit()
...
__CONTRIBUTORS__ = ['Abhay', 'Shreyas', 'Anary', 'Mayank']
...
description = """CEbomb - Your Friendly Spammer Application

CEbomb can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

CEbomb is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by SpeedX !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start CEbomb with SMS Bomb mode")
parser.add_argument("-ascii", "--ascii", action="store_true",
                    help="show only characters of standard ASCII set")
parser.add_argument("-u", "--update", action="store_true",
                    help="update CEbomb")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current CEbomb contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current CEbomb version")
